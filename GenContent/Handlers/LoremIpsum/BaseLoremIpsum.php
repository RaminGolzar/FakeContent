...
class BaseLoremIpsum
{
    ...
    /**
     * This property contains long text to generate lorem ipsum
     *
     * @var type
     */
    protected $text = <<<id
    ...
    /**
     * To hold length of "text" property
     *
     * @var int
     */
    protected const TEXT_LENGTH = 6869;

    /* ------------------------------------------------------------ */

    /**
     * For setting start & end string, on result text
     *
     * @param string $type <p>Allowed: "char" , "word"</p>
     * @param string $string
     * @return void
     */
    protected function finally_prepare (string $type , string &$string): void {
        $this->set_start ($type , $string);

        $this->set_end ($string);
    }

    /**
     * Setting "Lorem ipsum" text to begin result
     *
     * @param string $type
     * @param string $string
     * @return void
     */
    private function set_start (string &$type , string &$string): void {
        switch ($type) {
            case 'char' :
                $this->with_char ($string);
                break;
            case 'word':
                $this->with_word ($string);
                break;
        }
    }

    /**
     * Set a string to begin result, with character count
     *
     * @param string $string
     * @param bool $force
     * @return void
     */
    private function with_char (string &$string): void {
        if (strlen ($string) > 11) {
            $string = substr_replace ($string , 'Lorem ipsum ' , 0 , 11);
        }
    }

    /**
     * Set a string to begin result, with word count
     *
     * @param string $string
     * @return void
     */
    private function with_word (string &$string): void {
    ...
    /**
     * Set a dot sign to end of result
     *
     * @param string $string
     * @return string
     */
    private function set_end (string &$string): void {
        $string = substr_replace ($string , '.' , -1 , 1);
    }

    /**
     * Geting result length
     *
     * @param int $min
     * @param int|null $max
     * @return int
     */
    protected function get_length (int $min , ?int $max): int {
        if (!$max) {
            return $min;
        } else {
